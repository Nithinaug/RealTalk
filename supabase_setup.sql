...
CREATE POLICY "Allow sender to delete for everyone" ON messages FOR DELETE USING (auth.uid() = user_id);
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'user_rooms') THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE user_rooms;
    END IF;
